...
if [ $FORCE_BOOTSTRAP -eq 0 ]; then
  ...
  docker run -t --rm --user 0:1000 -v "${CODEBASE_DIR}:${CODEBASE_DIR}" -w "${CODEBASE_DIR}" ${DOCKER_DEV_IMAGE} python3.8 "${CODEBASE_DIR}/tests/resources/check_integrity.py"
  integrityResult="$?"
...
if [ $integrityResult -ne 0 ]; then
  docker run -t --rm --user 0:1000 -e "PYTHONPATH=${CODEBASE_DIR}" -v "${CODEBASE_DIR}:${CODEBASE_DIR}" -w "${CODEBASE_DIR}" ${DOCKER_DEV_IMAGE} python3.8 "${CODEBASE_DIR}/tests/resources/bootstrap.py"
...
docker run -t --rm --user 0:1000 ${CPU_TEST} --shm-size=8g -v "${CODEBASE_DIR}:${CODEBASE_DIR}" -w "${CODEBASE_DIR}" ${DOCKER_DEV_IMAGE} python3.8 -m pytest "${CODEBASE_DIR}/tests"
